...
@pytest.mark.asyncio
async def test_not_editable_message(mocker: MockerFixture, caplog: LogCaptureFixture, client, message):
    ...
    assert (
        'Источник 0 изменил сообщение 0, оно не может быть изменено в категории, потому что было '
        'переслано и не перепечатывалось'
    ) in caplog.text
