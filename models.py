...
class Source(BaseModel):
    ...
    def get_blacklist_hashtag(self) -> list:
        query = Filter.filter(content_type='hashtag', channel=self)
        return [blacklist.pattern for blacklist in query]

    def get_blacklist_part_of_url(self) -> list:
        query = Filter.filter(content_type='part_of_url', channel=self)
        return [blacklist.pattern for blacklist in query]

    def get_blacklist_part_of_text(self) -> list:
        query = Filter.filter(content_type='part_of_text', channel=self)
        return [blacklist.pattern for blacklist in query]

    def get_blacklist_reply_markup(self) -> list:
        query = Filter.filter(content_type='reply_markup', channel=self)
        return [blacklist.pattern for blacklist in query]
...
class Filter(BaseModel):
    ...
    def global_hashtag() -> list:
        query = Filter.filter(content_type='hashtag', channel=None)
        return [blacklist.pattern for blacklist in query]

    @staticmethod
    def global_part_of_url() -> list:
        query = Filter.filter(content_type='part_of_url', channel=None)
        return [blacklist.pattern for blacklist in query]

    @staticmethod
    def global_part_of_text() -> list:
        query = Filter.filter(content_type='part_of_text', channel=None)
        return [blacklist.pattern for blacklist in query]

    @staticmethod
    def global_reply_markup() -> list:
        query = Filter.filter(content_type='reply_markup', channel=None)
        return [blacklist.pattern for blacklist in query]
