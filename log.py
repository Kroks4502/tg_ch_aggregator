...
handler = RotatingFileHandler(os.path.join(
    LOGS_DIR, 'app.log'), maxBytes=50000000, backupCount=5)
handler.setFormatter(logging.Formatter(LOG_FORMAT))
logger.addHandler(handler)

peewee_logger = logging.getLogger('peewee')
peewee_logger.setLevel(logging.WARNING)
peewee_handler = RotatingFileHandler(os.path.join(
    LOGS_DIR, 'peewee.log'), maxBytes=50000000, backupCount=5)
...
pyrogram_handler = RotatingFileHandler(os.path.join(
    LOGS_DIR, 'pyrogram.log'), maxBytes=50000000, backupCount=5)
